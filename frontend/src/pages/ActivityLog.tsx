...
import { useLoggedExpenses, addLoggedExpense, LoggedExpense } from "@/pages/Dashboard";
import { formatCurrency } from "@/lib/format";
import { Receipt, Clock, TrendingUp, ArrowUpRight, CreditCard, Smartphone, Banknote, Plus, X, Trash2, Search } from "lucide-react";
...
const EXPENSE_CATEGORIES = ["Food & Dining", "Transport", "Entertainment", "Bills & Utilities", "Shopping", "Other"];
const PAYMENT_METHODS_LIST = ["Credit Card", "Debit Card", "UPI", "Bank Transfer", "Cash"];
...
export default function ActivityLog() {
    ...
    (async () => {
      try {
        ...
        if (response.ok) {
          ...
          setServerExpenses(data.map((e: any) => ({
            id: e.id,
            ...
            source: e.added_via === "voice" ? "voice" as const : "text" as const,
            paymentMethod: e.payment_method,
          })));
        }
      } catch { /* offline — show only locally-logged expenses */ }
    })();
  }, [refreshTrigger]);
  ...
  const allExpenses = useMemo(() => {
    ...
    const serverFiltered = serverExpenses.filter(e => !loggedIds.has(e.id) && !deletedIds.has(e.id));
    return [...loggedExpenses.filter(e => !deletedIds.has(e.id)), ...serverFiltered].sort(
  ...
  const [newExpense, setNewExpense] = useState({
    description: "", amount: "", categories: ["Food & Dining"] as string[],
    date: new Date().toISOString().split("T")[0], paymentMethod: "Cash"
  });
  const [splitAmounts, setSplitAmounts] = useState<Record<string, string>>({});
  const isMultiCategory = newExpense.categories.length > 1;

  const [activeTab, setActiveTab] = useState<"expenses" | "conversations">("expenses");
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return result;
  }, [allExpenses, searchText, filterCategory, filterSource, filterDateFrom, filterDateTo]);

  const hasActiveFilters = searchText || filterCategory !== "All" || filterSource !== "All" || filterDateFrom || filterDateTo;

  const clearAllFilters = () => {
    setSearchText(""); setFilterCategory("All"); setFilterSource("All");
  ...
  const [convTypeFilter, setConvTypeFilter] = useState<"all" | "voice" | "text">("all");
  ...
  const hasVoiceMessages = (conv: any) =>
    ...
    conv.messages.some((m: any) => m.source === "voice" || (m.id || "").startsWith("vapi-"));
  ...
  const deleteConversation = async (id: string) => {
    ...
    try {
      await authFetch(`${API_BASE_URL}/conversations/${id}`, { method: "DELETE" });
  ...
  const handleAddExpense = async () => {
    ...
    if (isMultiCategory) {
      ...
      if (Math.abs(splitTotal - amount) > 0.01) {
        toast.error(`Split amounts (${splitTotal.toFixed(2)}) must equal total (${amount.toFixed(2)})`);
        return;
      }

      try {
        const response = await authFetch(`${API_BASE_URL}/expenses/split`, {
          method: "POST",
        ...
        if (response.ok) {
          ...
          toast.success(`Split expense saved across ${newExpense.categories.length} categories`);
        } else {
      ...
      try {
        await authFetch(`${API_BASE_URL}/expenses/`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    setNewExpense({ description: "", amount: "", categories: ["Food & Dining"], date: new Date().toISOString().split("T")[0], paymentMethod: "Cash" });
    setSplitAmounts({});
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Your confirmed expense history</p>
        </div>
        <button onClick={() => setShowAddForm(v => !v)} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors">
      ...
      {showAddForm && (
        <div className="bg-card border border-border rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-medium text-foreground">New Expense</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input type="text" placeholder="Description" value={newExpense.description} onChange={e => setNewExpense(p => ({...p, description: e.target.value}))} className="text-sm bg-card border border-input rounded-lg px-3 py-2 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
            <input type="number" placeholder="Amount" value={newExpense.amount} onChange={e => setNewExpense(p => ({...p, amount: e.target.value}))} className="text-sm bg-card border border-input rounded-lg px-3 py-2 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
            <div className="text-sm bg-card border border-input rounded-lg px-3 py-2 text-foreground space-y-1">
              <p className="text-xs text-muted-foreground mb-1">Categories (select one or more)</p>
                ...
                <label key={c} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => {
                      ...
                      if (!checked) {
                        setSplitAmounts(prev => { const next = {...prev}; delete next[c]; return next; });
                      }
                    }}
            ...
            {isMultiCategory && (
              <div className="sm:col-span-2 space-y-2">
                <p className="text-xs text-muted-foreground">Split amounts by category (must total {newExpense.amount || "0"})</p>
                {newExpense.categories.map(cat => (
                  <div key={cat} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={e => setSplitAmounts(prev => ({...prev, [cat]: e.target.value}))}
                ...
                {(() => {
                  ...
                  if (totalAmt > 0 && diff > 0.01) {
                    return <p className="text-xs text-destructive">Split total ({splitTotal.toFixed(2)}) ≠ expense total ({totalAmt.toFixed(2)}). Difference: {diff.toFixed(2)}</p>;
                  }
                  return null;
                })()}
              </div>
            )}
            <select value={newExpense.paymentMethod} onChange={e => setNewExpense(p => ({...p, paymentMethod: e.target.value}))} className="text-sm bg-card border border-input rounded-lg px-3 py-2 text-foreground">
              {PAYMENT_METHODS_LIST.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
            <input type="date" value={newExpense.date} onChange={e => setNewExpense(p => ({...p, date: e.target.value}))} className="text-sm bg-card border border-input rounded-lg px-3 py-2 text-foreground" />
          </div>
          <div className="flex gap-2">
            <button onClick={handleAddExpense} className="px-4 py-2 text-xs font-medium bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors">Save</button>
            <button onClick={() => setShowAddForm(false)} className="px-4 py-2 text-xs font-medium text-muted-foreground hover:bg-muted rounded-lg transition-colors">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-card rounded-xl border border-border p-3 space-y-2.5">
        <div className="relative">
          ...
          {searchText && (
            <button onClick={() => setSearchText("")} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted-foreground hover:text-foreground">
      ...
      {grouped.map(([date, expenses]) => (
        <div key={date}>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{formatDate(date)}</h3>
            <span className="text-xs text-muted-foreground">{formatCurrency(expenses.reduce((s, e) => s + e.amount, 0))}</span>
          </div>
          <div className="space-y-2">
            {expenses.map(entry => {
              const PayIcon = paymentIcons[entry.paymentMethod || ""] || ArrowUpRight;
              return (
                <div key={entry.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${categoryColors[entry.category] || categoryColors["Other"]}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-xl border border-border p-3 space-y-2.5">
            <div className="relative">
              ...
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
              {convSearchText && (
                <button onClick={() => setConvSearchText("")} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted-foreground hover:text-foreground">
                  <X className="w-3.5 h-3.5" />
                </button>
              )}
            </div>
            ...
            {(convSearchText || convDateFrom || convDateTo || convTypeFilter !== "all") && (
              <div className="flex items-center justify-between pt-1">
                <span className="text-xs text-muted-foreground">
                  {filteredConversations.length} of {conversations.length} sessions
                </span>
                <button onClick={() => { setConvSearchText(""); setConvDateFrom(""); setConvDateTo(""); setConvTypeFilter("all"); }} className="text-xs text-primary hover:underline">
                  Clear filters
                </button>
              </div>
            )}
          ...
          {filteredConversations.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-8 text-center">
              <p className="text-sm text-muted-foreground">No Guru sessions saved yet.</p>
              <p className="text-xs text-muted-foreground mt-1">Switch to Guru mode in the Assistant, have a conversation, then save it to see it here.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {filteredConversations.map(conv => (
                <div key={conv.id} className="bg-card border border-border rounded-xl p-4 hover:bg-muted/30 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <button onClick={() => setSelectedConversation(conv)} className="flex-1 text-left min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {new Date(conv.updated_at).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                      </p>
                    </button>
                    <button onClick={() => deleteConversation(conv.id)} className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" aria-label="Delete conversation">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
          ...
          {selectedConversation && (
            <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedConversation(null)}>
              <div className="bg-card border border-border rounded-2xl max-w-2xl w-full max-h-[80vh] flex flex-col shadow-xl" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between p-4 border-b border-border">
                  <div>
                    <p className="text-sm font-semibold text-foreground">{selectedConversation.title}</p>
                    <p className="text-xs text-muted-foreground">
                      {selectedConversation.mode === "guru" ? "Guru" : "SmartSpend"} · {selectedConversation.message_count} messages
                    </p>
                  </div>
                  <button onClick={() => setSelectedConversation(null)} className="p-1.5 rounded-lg hover:bg-muted transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {selectedConversation.messages.map((msg: any, i: number) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-xs ${msg.role === "user" ? (msg.is_guru ? "bg-amber-500 text-white" : "bg-primary text-primary-foreground") : "bg-muted text-foreground"}`}>
                        {msg.content}
                      </div>
                    </div>
                  ))}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              "{deleteTarget?.description}" will be permanently removed and your budget totals will be recalculated. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
